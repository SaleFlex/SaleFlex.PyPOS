...
- **`ensure_settings_hub_layout`** (`data_layer/db_init_data/forms/setting_form.py`) migrates form **#3** to **SETTINGS_MENU** (hub), ensures **#23 POS_SETTINGS** and **#24 LOYALTY_SETTINGS**, and removes obsolete **Main Menu** / settings **CAMPAIGNS** shortcut controls — see [Configuration](04-configuration.md).
- **`ensure_campaign_management_forms`** (`data_layer/db_init_data/forms/campaign_management.py`) adds **CAMPAIGN_LIST** / **CAMPAIGN_DETAIL** when missing and patches detail **TABCONTROL** when needed — see [Dynamic Forms — Campaign management](22-dynamic-forms-system.md#campaign-management-administrators).
