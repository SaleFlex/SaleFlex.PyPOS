...
- **`ensure_campaign_management_forms`** (`data_layer/db_init_data/forms/campaign_management.py`) adds **CAMPAIGN_LIST** / **CAMPAIGN_DETAIL** when missing, **`GOTO_CAMPAIGNS`** on the main menu, and **`GOTO_CAMPAIGNS_FROM_SETTINGS`** on **SETTING** — see [Dynamic Forms — Campaign management](22-dynamic-forms-system.md#campaign-management-administrators).
