...
- **Customer Detail** modal — **Activity History** tab (`CUSTOMER_ACTIVITY_GRID`): `DynamicDialog` populates from `TransactionHead` by `fk_customer_id`; **Point movements** tab (`CUSTOMER_LOYALTY_POINTS_GRID`): `loyalty_point_transaction` audit (see [Customer Management](17-customer-management.md))
