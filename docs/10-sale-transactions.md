...
**Promotional document discounts (when present):** **`CampaignService.evaluate_proposals`** can compute eligible discounts from **`Campaign`** data (read-only **`CampaignDiscountProposal`** list); the POS does not yet turn those proposals into **`TransactionDiscountTemp`** rows on SALE. A GATE or custom integration may also add temp rows with **`discount_type="CAMPAIGN"`** and **`discount_code`** set to **`Campaign.code`** or a short coupon token (column limit 15). There is no dedicated SALE/PAYMENT button for promotions yet. **`PaymentService.copy_temp_to_permanent`** resolves the **`CAMPAIGN`** **`transaction_discount_type`** row and copies **`discount_code`** to **`TransactionDiscount`**; the thermal receipt shows **`CAMPAIGN (code)`** — see [Campaign & Promotions](43-campaign-promotions.md).
