...
**Promotional document discounts (when present):** On **SALE**, **`sync_campaign_discounts_on_document`** runs after adding a line (**`SaleService.add_sale_to_document`**) and when the cart changes in ways that affect eligibility (**`SaleService.refresh_campaign_discounts_after_cart_change`**: line discount/markup, **DELETE** / **REPEAT**, customer assignment, successful **COUPON** apply). It cancels prior engine-owned **`CAMPAIGN`** temp rows, merges **`active_coupon_codes`** (optional) with codes from **`Coupon`** rows referenced by **`document_data["applied_coupon_ids"]`**, calls **`CampaignService.evaluate_proposals`** (campaign definitions from **`ActiveCampaignCache`** when loaded, with **`CampaignUsageLimits`** for **`total_usage_limit`** / **`usage_limit_per_customer`**), inserts new **`TransactionDiscountTemp`** rows, and recomputes **`head.total_discount_amount`**. This is **skipped** when **`gate.manages_campaign`** is true (GATE is expected to supply campaign lines); the **COUPON** button then only shows an informational message. A GATE or third-party connector may also add temp rows with **`discount_type="CAMPAIGN"`** and **`discount_code`** (column limit 15). **`PaymentService.copy_temp_to_permanent`** runs **`CampaignAuditService.record_after_completed_sale`** (**`CampaignUsage`**, **`CouponUsage`**, campaign/coupon counters when applicable), resolves the **`CAMPAIGN`** **`transaction_discount_type`** row, and copies **`discount_code`** to **`TransactionDiscount`**; the thermal receipt shows **`CAMPAIGN (code)`** — see [Campaign & Promotions](43-campaign-promotions.md).
