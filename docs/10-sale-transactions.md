...
**Promotional document discounts (when present):** **`sync_campaign_discounts_on_document`** runs after adding a line on **SALE** (**`SaleService.add_sale_to_document`**) and when the cart changes in ways that affect eligibility (**`SaleService.refresh_campaign_discounts_after_cart_change`**: line discount/markup, **DELETE** / **REPEAT**, customer assignment, successful **COUPON** apply). It also runs after each successful **`PaymentService.process_payment`** so **payment-method** campaigns (`PAYMENT_DISCOUNT`) see **`TransactionPaymentTemp`** rows. It cancels prior engine-owned **`CAMPAIGN`** temp rows, merges **`active_coupon_codes`** (optional) with codes from **`Coupon`** rows referenced by **`document_data["applied_coupon_ids"]`**, calls **`CampaignService.evaluate_proposals`** (definitions from **`ActiveCampaignCache`** when loaded, with **`CampaignUsageLimits`** and **stacking** via **`priority`** / **`is_combinable`**), inserts new **`TransactionDiscountTemp`** rows (optional **`fk_transaction_payment_id`** for payment-linked promos), and recomputes **`head.total_discount_amount`**. Supported local **`CampaignType`** codes include basket, time-window, product, buy-X-get-Y, and payment-method discounts; line rules cover product, department, brand, barcode pattern, and category — see [Campaign & Promotions — Local evaluation](43-campaign-promotions.md#local-evaluation-campaignservice). This is **skipped** when **`gate.manages_campaign`** is true (GATE is expected to supply campaign lines); the **COUPON** button then only shows an informational message. A GATE or third-party connector may also add temp rows with **`discount_type="CAMPAIGN"`** and **`discount_code`** (column limit 15). **`PaymentService.copy_temp_to_permanent`** runs **`CampaignAuditService.record_after_completed_sale`** (**`CampaignUsage`**, **`CouponUsage`**, campaign/coupon counters when applicable), resolves the **`CAMPAIGN`** **`transaction_discount_type`** row, and copies **`discount_code`** to **`TransactionDiscount`**; the thermal receipt shows **`CAMPAIGN (code)`**. **`CampaignAuditService.distinct_applied_campaign_count(document_data)`** matches the distinct-campaign count used for **`CampaignUsage`** and aligns with **`CashierTransactionMetrics.number_of_promotions_applied`** when you persist transaction metrics — see [Campaign & Promotions](43-campaign-promotions.md).
