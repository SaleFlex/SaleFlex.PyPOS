...
- **Business Logic Layer** (`pos/service/`): Service classes (VatService, SaleService, PaymentService, LoyaltyEarnService, LoyaltyRedemptionService, LoyaltyService, CustomerSegmentService) and **`campaign/`** (cart snapshot, **`CampaignService`**, **`CampaignUsageLimits`**, **`CampaignAuditService`**, **`CouponActivationService`**, **`sync_campaign_discounts_on_document`**, **`SaleService.refresh_campaign_discounts_after_cart_change`** — see [docs/43-campaign-promotions.md](docs/43-campaign-promotions.md))
...
│ pos/service/campaign — snapshot, evaluate, sync, audit, coupons │
├─────────────────────────────────────────────────┤
...
│   │   └── campaign/          # cart_snapshot, campaign_service, campaign_document_sync, coupon_activation_service, campaign_audit_service, campaign_usage_limits, application_policy; docs/43
│   │
