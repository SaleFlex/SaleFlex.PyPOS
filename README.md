...
- **Business Logic Layer** (`pos/service/`): Service classes (VatService, SaleService, PaymentService, LoyaltyEarnService, LoyaltyRedemptionService, LoyaltyService, CustomerSegmentService) and **`campaign/`** (cart snapshot, **`ActiveCampaignCache`**, **`CampaignService`**, **`CampaignUsageLimits`**, **`CampaignAuditService`**, **`CouponActivationService`**, **`sync_campaign_discounts_on_document`**, **`SaleService.refresh_campaign_discounts_after_cart_change`** — see [docs/43-campaign-promotions.md](docs/43-campaign-promotions.md))
...
- [ ] **Tax & Discount Engine** - Advanced tax calculation and discount management (bulk pricing; **auto campaigns** on SALE for supported types — [docs/43-campaign-promotions.md](docs/43-campaign-promotions.md))
...
- [ ] **Campaign & Promotion Management**:
  ...
  - [x] **Local evaluation** — `CampaignService.evaluate_proposals` for `BASKET_DISCOUNT`, `TIME_BASED`, `PRODUCT_DISCOUNT` (+ rules / `CampaignProduct`); see [docs/43-campaign-promotions.md](docs/43-campaign-promotions.md#local-evaluation-campaignservice)
  - [x] **SALE document sync** — `sync_campaign_discounts_on_document` after line add and via `SaleService.refresh_campaign_discounts_after_cart_change` (line discount/markup, DELETE/REPEAT, customer assign, coupon apply); `head.total_discount_amount` recomputed from all active temp discounts; skipped when `gate.manages_campaign`; see [docs/43-campaign-promotions.md](docs/43-campaign-promotions.md#sale-document-sync-local-engine)
  ...
  - [ ] Priority / combinable rules UI (stacking already in `application_policy` / `CampaignService`)
