...
- **Campaign & Promotion Management**: DB models and seed campaigns (`Campaign`, `CampaignRule`, `CampaignProduct`, …). **`pos/service/campaign/`** provides a canonical **cart snapshot** (`schema_version` 1.0) for a future local engine and **GATE** (`CampaignSerializer.build_discount_request`), plus documented **stacking** (`priority`, `is_combinable`) and **loyalty** interaction. **`transaction_discount_type`** includes **`CAMPAIGN`** (fresh seeds + **`ensure_transaction_discount_type_campaign`** on startup for older DBs); completed sales map temp **`CAMPAIGN`** lines via **`PaymentService.copy_temp_to_permanent`**, with **`discount_code`** for **`Campaign.code`** or a short coupon token (15 characters). Thermal receipts label **`CAMPAIGN (code)`** in **`document_adapter`**. **SALE auto-apply is not wired yet** — see [docs/43-campaign-promotions.md](docs/43-campaign-promotions.md)
...
- **Business Logic Layer** (`pos/service/`): Service classes (VatService, SaleService, PaymentService, LoyaltyEarnService, LoyaltyRedemptionService, LoyaltyService, CustomerSegmentService) and **`campaign/`** (cart snapshot + promotion policy for GATE/local — see [docs/43-campaign-promotions.md](docs/43-campaign-promotions.md))
...
│ pos/service/campaign — cart snapshot v1         │
├─────────────────────────────────────────────────┤
...
│   │   └── campaign/          # Cart snapshot (schema 1.0), stacking policy; see docs/43-campaign-promotions.md
│   │
