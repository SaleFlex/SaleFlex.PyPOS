...
- **Campaign & Promotion Management**: DB models and seed campaigns (`Campaign`, `CampaignRule`, `CampaignProduct`, …). **`pos/service/campaign/`** provides a canonical **cart snapshot** (`schema_version` 1.0) for **GATE** (`CampaignSerializer.build_discount_request`), documented **stacking** (`priority`, `is_combinable`), **`CampaignService.evaluate_proposals`** (**basket / time / product** types), and **`sync_campaign_discounts_on_document`**, which applies proposals to the **open SALE** as **`TransactionDiscountTemp`** rows (`discount_type` **`CAMPAIGN`**) whenever the cart or assigned customer changes (skipped when **`gate.manages_campaign`** is true). **`SaleService.refresh_campaign_discounts_after_cart_change`** refreshes the sale list and amount table after line discount/markup, **DELETE** / **REPEAT**, and customer assignment. **`transaction_discount_type`** includes **`CAMPAIGN`** (fresh seeds + **`ensure_transaction_discount_type_campaign`** on startup for older DBs); **`PaymentService.copy_temp_to_permanent`** maps temp lines with **`discount_code`** (15 characters). Thermal receipts label **`CAMPAIGN (code)`** in **`document_adapter`**. **`IntegrationMixin.apply_campaign`** adds **`campaign_proposals`** to **`document_data`-shaped** carts when GATE and the third-party campaign connector are off — see [docs/43-campaign-promotions.md](docs/43-campaign-promotions.md)
...
- **Business Logic Layer** (`pos/service/`): Service classes (VatService, SaleService, PaymentService, LoyaltyEarnService, LoyaltyRedemptionService, LoyaltyService, CustomerSegmentService) and **`campaign/`** (cart snapshot, **`CampaignService`**, **`sync_campaign_discounts_on_document`**, **`SaleService.refresh_campaign_discounts_after_cart_change`** — see [docs/43-campaign-promotions.md](docs/43-campaign-promotions.md))
...
│   │   └── campaign/          # cart_snapshot, campaign_service, campaign_document_sync, application_policy; docs/43
│   │
