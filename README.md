...
- **Business Logic Layer** (`pos/service/`): Service classes (VatService, SaleService, PaymentService, LoyaltyEarnService, LoyaltyRedemptionService, LoyaltyService, CustomerSegmentService) and **`campaign/`** (cart snapshot, **`CampaignService`**, **`CouponActivationService`**, **`sync_campaign_discounts_on_document`**, **`SaleService.refresh_campaign_discounts_after_cart_change`** — see [docs/43-campaign-promotions.md](docs/43-campaign-promotions.md))
...
│ pos/service/campaign — snapshot + CampaignService │
├─────────────────────────────────────────────────┤
...
│   │   └── campaign/          # cart_snapshot, campaign_service, campaign_document_sync, coupon_activation_service, application_policy; docs/43
│   │
...
- [ ] **Campaign & Promotion Management**:
  ...
  - [x] **Coupon activation on SALE** — `CouponActivationService` + `document_data["applied_coupon_ids"]`; **COUPON** button / `APPLY_COUPON`; `CouponUsage` + usage counters on completed sale; demo seed `WELCOME10-DEMO` + `ensure_sample_coupon_welcome_demo` / `ensure_sale_form_coupon_button` / `ensure_welcome10_demo_campaign_product` on startup — [docs/43-campaign-promotions.md](docs/43-campaign-promotions.md#coupon-activation-on-sale)
