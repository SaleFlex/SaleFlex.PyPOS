...
- **Campaign & Promotion Management**: DB models and seed campaigns (`Campaign`, `CampaignRule`, `CampaignProduct`, …). **`pos/service/campaign/`** provides a canonical **cart snapshot** (`schema_version` 1.0) for **GATE** (`CampaignSerializer.build_discount_request`), documented **stacking** (`priority`, `is_combinable`), and **`CampaignService.evaluate_proposals`** (read-only evaluation for **basket / time / product** campaign types). **`transaction_discount_type`** includes **`CAMPAIGN`** (fresh seeds + **`ensure_transaction_discount_type_campaign`** on startup for older DBs); completed sales map temp **`CAMPAIGN`** lines via **`PaymentService.copy_temp_to_permanent`**, with **`discount_code`** for **`Campaign.code`** or a short coupon token (15 characters). Thermal receipts label **`CAMPAIGN (code)`** in **`document_adapter`**. **Applying proposals to the open SALE document is not wired yet** — see [docs/43-campaign-promotions.md](docs/43-campaign-promotions.md)
...
- **Business Logic Layer** (`pos/service/`): Service classes (VatService, SaleService, PaymentService, LoyaltyEarnService, LoyaltyRedemptionService, LoyaltyService, CustomerSegmentService) and **`campaign/`** (cart snapshot, **`CampaignService.evaluate_proposals`**, policy for GATE/local — see [docs/43-campaign-promotions.md](docs/43-campaign-promotions.md))
...
│   │   └── campaign/          # cart_snapshot, campaign_service (evaluate_proposals), application_policy; docs/43
│   │
...
- [ ] **Campaign & Promotion Management**:
  ...
  - [x] **Local evaluation** — `CampaignService.evaluate_proposals` for `BASKET_DISCOUNT`, `TIME_BASED`, `PRODUCT_DISCOUNT` (+ rules / `CampaignProduct`); not wired to SALE temp discounts yet — [docs/43-campaign-promotions.md](docs/43-campaign-promotions.md#local-evaluation-campaignservice)
