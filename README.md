...
- **Campaign & Promotion Management**: DB models and seed campaigns (`Campaign`, `CampaignRule`, `CampaignProduct`, **`Coupon`**, **`CouponUsage`**, **`CampaignUsage`**, …). **Administrators** open **CAMPAIGN_LIST** / **CAMPAIGN_DETAIL** from **Main Menu → SETTINGS → CAMPAIGN SETTINGS** (dynamic forms **#21** / **#22** in `data_layer/db_init_data/forms/campaign_management.py`; **`ensure_campaign_management_forms`** adds missing pieces on startup; **SAVE** on the detail dialog calls **`refresh_active_campaign_cache()`**; **`CAMPAIGN_LIST_FORM`** controls are hidden for non-administrators — [Dynamic Forms — Campaign management](docs/22-dynamic-forms-system.md#campaign-management-administrators)). **`pos/service/campaign/`** provides a canonical **cart snapshot** (`schema_version` 1.0) for **GATE** (`CampaignSerializer.build_discount_request`), documented **stacking** (`priority`, `is_combinable`), **`ActiveCampaignCache`** (in-memory campaign definitions for **`CampaignService`**; warm-up at startup; refresh after GATE pull / **`campaign_update`** path / **`CampaignSerializer.apply_updates`**; **`CacheManager.refresh_active_campaign_cache()`** for admin saves), **`CampaignUsageLimits`** (**`total_usage_limit`**, **`usage_limit_per_customer`** via **`CampaignUsage`** counts), **`CampaignService.evaluate_proposals`** supports **`BASKET_DISCOUNT`**, **`TIME_BASED`**, **`PRODUCT_DISCOUNT`**, **`BUY_X_GET_Y`**, and **`PAYMENT_DISCOUNT`** (line rules: product, department, brand, barcode pattern, category; payment promos use **`PAYMENT_TYPE`** rules mapped from tender **`EventName`** to **`PaymentType`**). **Stacking** follows **`priority`** (descending) and **`is_combinable`** with a greedy pass that reduces eligible merchandise net after earlier proposals in the same evaluation. **`CouponActivationService`** (validate **`Coupon`** by code or barcode for **`Campaign.requires_coupon`**; **`PERSONAL`** / **`SINGLE_USE`** / **`usage_limit`** checks; append **`applied_coupon_ids`** on the open document), and **`sync_campaign_discounts_on_document`**, which merges coupon-derived campaign codes with **`active_coupon_codes`** and applies proposals to the **open SALE** as **`TransactionDiscountTemp`** rows (`discount_type` **`CAMPAIGN`**) when the cart, assigned customer, or **tender lines** change (**`PaymentService.process_payment`** re-runs sync so payment-based promos apply; skipped when **`gate.manages_campaign`** is true). **SALE** form **COUPON** button (**`APPLY_COUPON`**) opens a dialog (numpad text pre-fills when present); GATE-managed campaign mode shows an info message instead. **`SaleService.refresh_campaign_discounts_after_cart_change`** refreshes the sale list and amount table after line discount/markup, **DELETE** / **REPEAT**, customer assignment, and successful coupon apply. **`PaymentService.copy_temp_to_permanent`** calls **`CampaignAuditService.record_after_completed_sale`** (**`CampaignUsage`** audit row per campaign on the receipt, **`CouponUsage`**, **`Campaign.total_usage_count`**, **`Coupon.usage_count`**), then maps temp lines with **`discount_code`** (15 characters). **`CampaignAuditService.distinct_applied_campaign_count(document_data)`** matches that distinct-campaign count for **`CashierTransactionMetrics.number_of_promotions_applied`** when transaction metrics are persisted. **`CampaignAuditService.revoke_entitlements_for_transaction_head`** is available for void/refund flows that must roll back those entitlements. Thermal receipts label **`CAMPAIGN (code)`** in **`document_adapter`**. **`IntegrationMixin.apply_campaign`** adds **`campaign_proposals`** to **`document_data`-shaped** carts when GATE and the third-party campaign connector are off — see [docs/43-campaign-promotions.md](docs/43-campaign-promotions.md)
...
- **Application Layer** (`pos/manager/application.py`): Main application class implementing Singleton pattern, combining CurrentStatus, CurrentData, EventHandler, and **IntegrationMixin** (`init_integration()` after closure load; **`apply_campaign`** routing — see [docs/40-integration-layer.md](docs/40-integration-layer.md))
...
├─────────────────────────────────────────────────┤
│      Event Handlers (specialized mixins)        │
...
│ pos/service/campaign                            │
│ snapshot, cache, evaluate, sync, audit, coupons │
├─────────────────────────────────────────────────┤
...
│   │   │   ├── management.py # Form rows #3 SETTINGS_MENU, #4 CASHIER, #23–#24 POS/LOYALTY settings
│   │   │   ├── setting_form.py # Hub (#3), POS_SETTINGS (#23), LOYALTY_SETTINGS (#24); `ensure_settings_hub_layout`
...
│   │   └── campaign/          # cart_snapshot, active_campaign_cache, campaign_service, campaign_document_sync, coupon_activation_service, campaign_audit_service, campaign_usage_limits, application_policy; docs/43
│   │
...
> Main Menu listing all major modules: Sales, Closure, Settings, Cashier Management, Products, Warehouse, Customer, and Logout. Campaign administration is under **Settings → CAMPAIGN SETTINGS** (not a separate Main Menu button).
...
- **Campaign & Promotion**: CampaignType, Campaign, CampaignRule, CampaignProduct, CampaignUsage, Coupon, CouponUsage — runtime snapshot contract: [docs/43-campaign-promotions.md](docs/43-campaign-promotions.md)
...
- [x] **Event Handler System** - Comprehensive event handling with specialized event mixins (`GeneralEvent`, `SaleEvent`, `PaymentEvent`, `ClosureEvent`, `ConfigurationEvent`, `ServiceEvent`, `ReportEvent`, `HardwareEvent`, `WarehouseEvent`, `ProductEvent`, `CustomerEvent`, `CampaignEvent`) for modular organization. All event handler methods use `_event` suffix (e.g., `_sales_form_event`, `_closure_event`, `_campaign_search_event`) to avoid conflicts with properties. Customer events include `CUSTOMER_ADD` (create new customer) and `CUSTOMER_LIST_BACK` (context-aware BACK that assigns the chosen customer to the active sale transaction). Campaign admin UI events: `CAMPAIGN_LIST_FORM`, `CAMPAIGN_SEARCH`, `CAMPAIGN_DETAIL`, `CAMPAIGN_DETAIL_SAVE` (`pos/manager/event/campaign.py`)
...
- [ ] **Tax & Discount Engine** - Advanced tax calculation and discount management (bulk pricing; **auto campaigns** on SALE / PAYMENT sync for basket, time-window, product, buy-X-get-Y, and payment-method types — [docs/43-campaign-promotions.md](docs/43-campaign-promotions.md))
...
### Business Features
- [x] **Campaign & Promotion Management** (local engine, sync, coupons, admin UI; gaps in sub-items):
  - [x] **Cart snapshot & policy** — `pos/service/campaign/` (`schema_version` 1.0, `normalize_cart_data_for_campaign_request`, `CampaignSerializer.build_discount_request` body, documented stacking vs loyalty); see [docs/43-campaign-promotions.md](docs/43-campaign-promotions.md)
  - [x] **Active campaign cache** — `ActiveCampaignCache` + `CacheManager.refresh_active_campaign_cache()`; warm at startup; reload after `hooks.pull_updates_from_gate`, `SyncWorker` pull, `CampaignSerializer.apply_updates`; see [docs/43-campaign-promotions.md](docs/43-campaign-promotions.md#active-campaign-cache)
  - [x] **`CAMPAIGN` discount type** — `transaction_discount_type` seed row, `ensure_transaction_discount_type_campaign` on startup for existing DBs, `PaymentService.copy_temp_to_permanent` mapping, `discount_code` for campaign/coupon id (15 char), thermal receipt line in `document_adapter`; see [docs/43-campaign-promotions.md](docs/43-campaign-promotions.md#campaign-transaction-discount-type)
  - [x] **Local evaluation** — `CampaignService.evaluate_proposals` for `BASKET_DISCOUNT`, `TIME_BASED`, `PRODUCT_DISCOUNT`, `BUY_X_GET_Y`, `PAYMENT_DISCOUNT` (extended `CampaignRule` matching, `CampaignProduct`, stacking); see [docs/43-campaign-promotions.md](docs/43-campaign-promotions.md#local-evaluation-campaignservice)
  - [x] **SALE / PAYMENT sync** — `sync_campaign_discounts_on_document` after line add, via `SaleService.refresh_campaign_discounts_after_cart_change` (line discount/markup, DELETE/REPEAT, customer assign, coupon apply), and after `PaymentService.process_payment` (payment-method promos); `head.total_discount_amount` recomputed from all active temp discounts; skipped when `gate.manages_campaign`; see [docs/43-campaign-promotions.md](docs/43-campaign-promotions.md#sale-document-sync-local-engine)
  - [x] **`apply_campaign` routing** — `Application` + `IntegrationMixin`, `init_integration()`; local fallback attaches `campaign_proposals` to `document_data`-shaped carts; `hooks.apply_campaign_discounts` calls `app.apply_campaign`; see [docs/40-integration-layer.md](docs/40-integration-layer.md#campaign-discount-routing)
  - [x] **Coupon activation on SALE** — `CouponActivationService` + `document_data["applied_coupon_ids"]`; **COUPON** button / `APPLY_COUPON`; `CampaignAuditService` (`CampaignUsage`, `CouponUsage`, counters); `CampaignUsageLimits`; `revoke_entitlements_for_transaction_head` for void/refund hooks; demo seed `WELCOME10-DEMO` + `ensure_sample_coupon_welcome_demo` / `ensure_sale_form_coupon_button` / `ensure_welcome10_demo_campaign_product` on startup — [docs/43-campaign-promotions.md](docs/43-campaign-promotions.md#coupon-activation-on-sale)
  - [x] **Campaign types (local engine)** — Product, basket, time-window, buy-X-get-Y, payment-method discounts; **Welcome Bonus** type not evaluated locally yet
  - [ ] **Free-product auto-apply** — percentage/fixed covered; automated free-line SKU flow not implemented
  - [x] **Time-based restrictions** — calendar date range, daily `start_time` / `end_time`, `days_of_week` on `Campaign` (shared by `TIME_BASED` and other types)
  - [x] **Eligibility rules** — product, department, brand, barcode pattern, category (sub-group UUID); payment-type rules for `PAYMENT_DISCOUNT`; customer segment via campaign FK (rule designer UI still optional)
  - [x] **Usage limits (global / per customer)** — `Campaign.total_usage_limit`, `Campaign.usage_limit_per_customer` enforced via `CampaignUsage` + `CampaignUsageLimits` in `CampaignService` and coupon validation; walk-in skips per-customer cap
  - [x] **Stacking (runtime)** — `priority` descending, `is_combinable`, greedy net adjustment in `CampaignService` / `application_policy.py`
  - [ ] **Priority / combinable admin UI** — edit in DB or forms only; dedicated stacking UI not required for runtime behaviour
  ...
  - [ ] **Coupon administration UI** — bulk issue, print, QR workflows beyond DB seeds (checkout activation covered above)
  - [x] **Campaign usage audit rows** — `CampaignUsage` + reversal API (`revoke_entitlements_for_transaction_head`); analytics dashboards still optional
  - [x] **Administrator campaign UI** — `CAMPAIGN_LIST` / `CAMPAIGN_DETAIL` forms, entry from **SETTINGS → CAMPAIGN SETTINGS**, `current_campaign_id`, SAVE → `refresh_active_campaign_cache()`; `ensure_campaign_management_forms`; see [docs/22-dynamic-forms-system.md](docs/22-dynamic-forms-system.md#campaign-management-administrators) and [docs/43-campaign-promotions.md](docs/43-campaign-promotions.md#management-ui-administrators)
...
| **[Configuration](docs/04-configuration.md)** | `settings.toml` + settings hub (`SETTINGS_MENU`, `POS_SETTINGS`, `LOYALTY_SETTINGS`) |
...
| **[Service Layer](docs/25-service-layer.md)** | VatService, SaleService, PaymentService, LoyaltyEarnService, LoyaltyRedemptionService, LoyaltyService, loyalty_settings_model (loyalty settings form), CustomerSegmentService |
