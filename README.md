...
- **Customer Management**: Store customer information, preferences, and purchase history. Fully operational Customer List search, Customer Detail view/edit, new-customer creation (ADD button), and Activity History via DB-driven dynamic forms. Walk-in Customer (`is_walkin = True`) placeholder automatically receives all unassigned sale transactions. The **SUB TOTAL** button on the SALE form is a **dual-function** button: in normal state it calculates the subtotal; after pressing **FUNC** it becomes **CUSTOMER** and opens the Customer List in *sale-assignment context* — selecting an existing customer or adding a new one automatically links them to the active sale transaction on BACK
...
- **Item Discount / Markup Buttons**: Two **dual-function** buttons on the SALE form (**DISC %** / **MARK %** and **DISC AMT** / **MARK AMT**) apply either a line discount or a line markup to the last sold item, depending on the button’s current state (press **FUNC** to flip all dual buttons to their alternate label without running an action, or tap the button to run the current action and cycle its label). A modal dialog opens with an editable amount field, embedded numeric keypad, optional on-screen **virtual keyboard** when the SALE window provides one, and **Enter** / **APPLY** to confirm. On apply, the original line is cancelled (strikethrough) and a new line is inserted with recalculated **unit price**, **total**, and **VAT**. Discounts persist `unit_discount`, `discount_rate`, and `discount_reason`; markups set `discount_reason` to a `Markup …` description and clear line discount fields. Percentage markup is 1–100 %; amount markup is from the smallest currency step up to the line’s current total. Currency `decimal_places` from the `Currency` table controls minimum step and precision. See [Sale Transactions](docs/10-sale-transactions.md#applying-item-discounts-and-markups)
...
- [x] **Item Discount / Markup Engine** - Dual-function DISC % / MARK % and DISC AMT / MARK AMT on the SALE form; percentage or fixed amount discount or markup on the last line; dialog with keypad, virtual keyboard when available, and Enter to apply; strikethrough cancel + new line with recalculated VAT
