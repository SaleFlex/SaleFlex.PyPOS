...
**SETTINGS:** [Configuration](docs/04-configuration.md) — **SETTINGS_MENU** hub, then **POS SETTINGS** and **LOYALTY SETTINGS** (tabbed loyalty); **SAVE** on each screen writes its panels. Existing DBs migrate via `ensure_settings_hub_layout` on startup.
...
**Campaigns** (snapshot, `ActiveCampaignCache`, SALE + PAYMENT sync, buy-X-get-Y, payment-method promos, stacking, limits, `CampaignUsage` audit, `distinct_applied_campaign_count` for metrics alignment, coupons, `apply_campaign`, **admin** UI under **Settings → CAMPAIGN SETTINGS**): [Campaign & Promotions](docs/43-campaign-promotions.md) · [Dynamic Forms — Campaign management](docs/22-dynamic-forms-system.md#campaign-management-administrators)
