...
**Campaigns:** Promotional **`Campaign`** rows and related tables exist in the database (seed examples; end dates are about **one year** from insert on new seeds). **Administrators** can open **CAMPAIGNS** from the **Main Menu** or **SETTINGS** to search campaigns, edit the selected row in **CAMPAIGN_DETAIL**, and **SAVE** (which runs **`refresh_active_campaign_cache()`**). Non-administrators do not see those navigation buttons. Details: [Dynamic Forms — Campaign management](docs/22-dynamic-forms-system.md#campaign-management-administrators). On the **SALE** screen, **`sync_campaign_discounts_on_document`** applies eligible **basket / time-window / product** discounts as **`TransactionDiscountTemp`** lines (`discount_type` **`CAMPAIGN`**) when you add or change lines, apply line discount/markup, use **DELETE** / **REPEAT**, or assign a customer; **`head.total_discount_amount`** is recomputed so the amount table matches. **`CampaignService.evaluate_proposals`** reads campaign definitions from **`ActiveCampaignCache`** when loaded (warm-up at app startup; refreshed after GATE pull / **`SyncWorker`** cycle / **`CampaignSerializer.apply_updates`**; use **`refresh_active_campaign_cache()`** if you change campaigns in the DB or a back-office tool). It applies **`CampaignUsageLimits`** (global and per-customer caps from **`CampaignUsage`**) and codes from **`active_coupon_codes`** (optional) plus **`Coupon`** rows for **`requires_coupon`** flows (**`document_data["applied_coupon_ids"]`** after **COUPON** / **`APPLY_COUPON`**). This path is **disabled** when **`gate.manages_campaign`** is true (GATE is expected to own pricing); the **COUPON** button then shows an informational message only. The **`pos/service/campaign/`** package also defines a **cart snapshot** (`schema_version` 1.0) for **GATE** (`CampaignSerializer.build_discount_request`). Stacking rules (`priority`, `is_combinable`) and how they relate to **line discounts** and **loyalty BONUS** are documented there. The reference table **`transaction_discount_type`** includes **`CAMPAIGN`**; missing rows are added on startup by **`ensure_transaction_discount_type_campaign`**. **`PaymentService.copy_temp_to_permanent`** runs **`CampaignAuditService.record_after_completed_sale`** (**`CampaignUsage`**, **`CouponUsage`**, counters) when a completed sale has **CAMPAIGN** discounts, then copies promotion lines to permanent discounts; the log/thermal receipt shows **`CAMPAIGN (code)`** like **`LOYALTY`**. Void/refund flows that must undo entitlements can call **`CampaignAuditService.revoke_entitlements_for_transaction_head`** once wired. **`IntegrationMixin.apply_campaign`** can attach a **`campaign_proposals`** list to **`document_data`-shaped** payloads when GATE and the third-party campaign connector are off. Full reference: [Campaign & Promotions](docs/43-campaign-promotions.md).
...
**Campaigns** (snapshot, `ActiveCampaignCache`, SALE sync, limits, `CampaignUsage` audit, coupons, `apply_campaign`, **admin CAMPAIGNS** list/detail UI): [Campaign & Promotions](docs/43-campaign-promotions.md) · [Dynamic Forms — Campaign management](docs/22-dynamic-forms-system.md#campaign-management-administrators)
